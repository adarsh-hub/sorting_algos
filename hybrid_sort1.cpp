...
void insertion_sort1(std::vector<int>& nums) {
	int i,j,temp;
	int size = nums.size();
	for(i = 1; i<size; i++){
		j=i;
...
std::vector<int> merge1(std::vector<int>& arr1, std::vector<int>& arr2)
{
    int i = 0, j = 0, k = 0;
    int n1 = arr1.size();
    int n2 = arr2.size();
...
void hybrid_sortr(std::vector<int>& nums, int H){
